...
- **Q1 (design) — "new" accent color. ✅ RESOLVED (BOOKSHELF-75): terracotta.** The unseen dot and "New" pill use a restrained terracotta accent — the `c-coral-*` scale added to `src/index.css` `@theme` and documented under Semantic colors in `docs/design-system.md`. It's the app's first accent beyond red (error) / green (success), signalling newness without colliding with either. Dot `bg-c-coral-500 dark:bg-c-coral-300`; pill `bg-c-coral-50 text-c-coral-800 border-c-coral-300` (dark: `bg-c-coral-900/30 text-c-coral-300 border-c-coral-800`).
