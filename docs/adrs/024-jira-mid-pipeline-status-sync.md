...
> **Correction (BOOKSHELF-84): `JIRA_BASE_URL` must be the gateway, not the site URL.** Atlassian's current API tokens are **scoped**, and scoped tokens only authenticate through `https://api.atlassian.com/ex/jira/{cloudId}` — the site URL `https://<site>.atlassian.net` returns `401` for a valid scoped token. So `JIRA_BASE_URL` is set to `https://api.atlassian.com/ex/jira/<cloudId>` (find `<cloudId>` at `https://<site>.atlassian.net/_edge/tenant_info`); the scripts prepend `/rest/api/3/…` to it, so **no code change**. This one variable fix also repairs ADR-022's prod `→ Done` sync, which had silently never worked for the same reason (both soft-failed, so it went unnoticed until this sync's self-test surfaced it). Full procedure — reusable for any repo/site — in `docs/runbooks/jira-ci-transitions.md`. (This supersedes the earlier "Basic auth is non-functional here" conclusion: Basic auth works fine; it was a wrong base URL.)
